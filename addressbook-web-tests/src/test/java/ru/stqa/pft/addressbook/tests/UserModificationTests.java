...
public class UserModificationTests extends TestBase {
  ...
  public void ensurePreconditions() {
    if (app.db().users().size() == 0) {
      ...
      app.user().create(new UserData()
              .withFirstname("Ron").withLastname("Weasley").withEmail("ronWeasley@magic.com").withCompany("").withPhoto(new File("src/test/resources/pft.png"))
  ...
  public void testUserModificationTest() throws Exception {
    ...
    UserData user = new UserData()
            .withId(modifiedUser.getId()).withFirstname("Hermine").withLastname("Granger").withEmail("herminegranger@magic.com").withCompany("")
            .inGroup(new GroupData().withFooter("").withHeader("").withName("test3"))
            //.withGroup("test1")
